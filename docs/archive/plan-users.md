...
> **Избыточно (2026-08-14):** `find_by_id` не нужен — `PlatformId::name()` константная функция, имя выводится из id без БД (`Platform::from_id(id)`), набор платформ фиксирован (twitch/youtube/vk_video_live), лишних id в системе не бывает. Заявлен в плане, в коде отсутствует, нигде не используется. Не реализовывать.
...
> **Избыточно:** тесты №3/№4 завязаны на удалённый `find_by_id` (см. таблицу репозитория) — не реализовывать.
...
- ~~`PlatformRepository::find_by_id(id)`~~ — **избыточно (2026-08-14):** заявлен в плане (таблица репозитория + тесты №3/№4), в коде отсутствует, нигде не используется. `PlatformId::name()` константная функция, имя выводится из id без БД; набор платформ фиксирован (twitch/youtube/vk_video_live), лишних id не бывает. Не реализовывать, оставлено в спецификации как отметка.
