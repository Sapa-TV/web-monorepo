...
- Шапка: заголовок «Админ-панель», root-badge, кнопка «Выйти» (переносятся из страницы).
- Сайдбар: пункты **Платформы**, **Виджеты**, **Действия и триггеры**, **Админы** с lucide-иконками (`~icons/lucide/*`). Активный пункт подсвечивается по `page.url.pathname` из `$app/state`; ссылки через `resolve()` по образцу остального кода.
