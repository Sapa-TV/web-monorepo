...
- [ ] Переделать `ActionExecutor`: сейчас это монолит со встроенной twitch-логикой (`ensure_user`, `send_chat_message`). Разделить на:
      ...
      - `PlatformActionExecutor` — по одному на платформу (twitch-исполнитель уже будет отдельным сервисом,
      дальше ютьюб/вк и т.д.), реально ходит в API платформы и возвращает результат экшену.
